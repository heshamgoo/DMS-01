...
export function EmployeeManagement() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const emps: EmployeeProfile[] = [];
      snapshot.forEach((doc) => {
        emps.push(doc.data() as EmployeeProfile);
      });
      setEmployees(emps);
      setLoading(false);
    }, (err) => {
  ...
  const handleAddOrUpdateEmployee = async (e: React.FormEvent) => {
    ...
    try {
      if (editingEmployee) {
        ...
        await updateDoc(doc(db, 'employees', editingEmployee.uid), updateData);
        await logActivity('Updated Employee', `Updated details for ${fullName} (${email})`);
      } else {
        // Create user in Firebase Auth using secondary app
        const userCredential = await createUserWithEmailAndPassword(secondaryAuth, email.toLowerCase(), password);
  ...
  const handleDelete = async (emp: EmployeeProfile) => {
    if (!window.confirm(`Are you sure you want to delete ${emp.fullName}? This action cannot be undone.`)) return;
    try {
      await deleteDoc(doc(db, 'employees', emp.uid));
      await logActivity('Deleted Employee', `Deleted account for ${emp.fullName} (${emp.email})`);
    } catch (err: any) {
