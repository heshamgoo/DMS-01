...
export function Settings() {
  ...
  useEffect(() => {
    if (settings) {
      ...
      setDepartments(settings.departments || ['General', 'HR', 'Finance', 'Operation', 'Public']);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await updateSettings({ 
        name, logoUrl, loginBgUrl, address, phone, email, logoTop, logoLeft, departments,
          ...
          <div className="pt-6 border-t border-gray-100">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {departments.map((dept) => (
                <span key={dept} className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                  {dept}
                  <button type="button" onClick={() => handleRemoveDept(dept)} className="ml-2 text-blue-600 hover:text-blue-800">
                    &times;
                  </button>
                </span>
              ))}
            ...
            <div className="flex space-x-2">
              <input
                ...
                className="flex-1 rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm py-2 border px-3"
