      ...
      try {
        if (this.state.error?.message) {
          ...
          if (parsed.error) {
            ...
            details = `Operation: ${parsed.operationType}, Path: ${parsed.path}. Error: ${parsed.error}`;
          }
        }
      } catch (e) {
      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
          <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8 text-center border border-red-100">
            ...
            <p className="text-slate-600 mb-6">{details || 'Please try refreshing the page or contact support if the problem persists.'}</p>
            <button
            ...
            {details && (
              <div className="mt-6 text-left">
                <p className="text-xs font-mono text-slate-400 break-all bg-slate-50 p-3 rounded border border-slate-100">
